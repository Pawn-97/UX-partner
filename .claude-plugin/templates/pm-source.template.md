...
valid_to: null
superseded_by: null
source_format: md      # md | docx
source_file: null      # DOCX 输入时填原始 .docx 路径；MD 输入留 null
assets_dir: null       # DOCX 输入时填 ./pm-source-assets；MD 输入留 null
---
...
- source_format: md | docx — DOCX 是经 pandoc 转换后的 markdown，原文档保留只读
- source_file: DOCX 输入时记原始路径，方便审计 / re-convert
- assets_dir: DOCX 输入时抽出来的图片目录（相对 pm-source.md 的路径）
